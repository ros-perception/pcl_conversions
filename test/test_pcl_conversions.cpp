...
protected:
  virtual void SetUp() {
    ...
    pcl_pc2.fields[0].datatype = sensor_msgs::PointField::INT8;
    pcl_pc2.fields[0].count = 3;
    pcl_pc2.fields[0].offset = 0;
    pcl_pc2.fields[1].name = "RGB";
    pcl_pc2.fields[1].datatype = sensor_msgs::PointField::INT8;
...
void test_pc(T &pc) {
  ...
  EXPECT_EQ(sensor_msgs::PointField::INT8, pc.fields[0].datatype);
  EXPECT_EQ(3, pc.fields[0].count);
  EXPECT_EQ(0, pc.fields[0].offset);
  EXPECT_EQ("RGB", pc.fields[1].name);
  EXPECT_EQ(sensor_msgs::PointField::INT8, pc.fields[1].datatype);
...
TEST_F(PCLConversionTests, pointcloud2Conversion) {
  ...
  test_pc(pc2);
  sensor_msgs::PointCloud2 pcl_pc2_2;
